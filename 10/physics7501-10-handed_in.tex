...
with the function $D$. We now need $D(\mathcal L) = d$ where $d = 6$ as the
integration over $\dif^6 x$ because $D(\int \dif^6 x) = -6$.

Looking at the bilinear mass term and taking $D(m) = 1$ for granted, we must
have $D(\phi) = 2$. From that we conclude $D(c) = 4$ and $D(g) = 0$. This is
all combined in Table~\ref{tab:mass_dimensions}.
...
\[
    D(m^2 \phi^2) = D(m^2) + D(\phi^2) = 2 + 2 D(\phi) \overset != d \,.
\]
From there we must have $D(\phi) = 2 - \epsilon$. With that we can look at $c$.
We have
\[
    D(c \phi) = D(c) + D(\phi) = D(c) + 2 - \epsilon \overset != d \,.
\]
We conclude that $D(c) = 4 - \epsilon$. Finally we can look at $g$. Here it is
\[
    D(g \phi^3) = D(g) + 3 D(\phi) = D(g) + 6 - 3 \epsilon \overset != d \,.
\]
Solving for $D(g)$ gives us $D(g) = \epsilon$. We now have to introduce the
$\mu$ to absorb the mass dimension of $g$ and make $g$ a dimensionless constant
again. We assume $D(\mu) = 1$ as $\mu$ has been used as a regulating photon
mass before. We need $D(\mu^x g) = \epsilon$ such that $D(g)$ can stay zero.
This brings us to
\[
    D(\mu^x g) = x D(\mu) + D(g) = x + 0 \overset != \epsilon
\]
...
\begin{table}
    ...
    \begin{tabular}{lSc}
        \toprule
        Term & {$D$ for $d = 6$} & {$D$ for $d = 6 - 2\epsilon$} \\
        \midrule
