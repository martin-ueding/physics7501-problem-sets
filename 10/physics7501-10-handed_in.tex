...
parameters that we are used to. We do this by splitting the constants into the
usual physical term and a counterterm. We do this using the following
definitions:
\[
    \delta_Z := Z - 1
    \eqnsep
...
derivation of those Feynman rules from the Lagrangian is as usual. We have
listed all of them in Table~\ref{tab:counterterms}.
...
\begin{table}[tbp]
    ...
    \begin{tabular}{ccc}
        ...
        \\ \midrule[0.02em]
        %
        $\displaystyle\frac12 \delta_Z [\partial \phir]^2$
        & \tdiagram{counter-dz} & \\ \midrule[0.02em]
        $\displaystyle-\delta_c \phir$
        & \tdiagram{counter-dc} & \\ \midrule[0.02em]
        $\displaystyle-\frac{\delta_m}2\phir^2$
        & \tdiagram{counter-dm} & \\ \midrule[0.02em]
